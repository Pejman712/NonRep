...
@dataclass
class KalmanState:
    """Kalman filter state for 4DOF transformation (translation + Z-rotation only)"""
    # State vector: [x, y, z, yaw]
    state: np.ndarray # 4x1 state vector
...
class PointCloudKalmanFilter:
    def __init__(self, dt: float = 5.0, process_noise: float = 0.5, measurement_noise: float = 0.01):
        ...
        MODIFIED: Only allows rotation around Z-axis (yaw), no roll or pitch
        Args:
        ...
        """
        self.dt = dt
        self.state_dim = 4  # [x,y,z,yaw] - no roll/pitch, no velocity terms needed
        self.measurement_dim = 4 # [x,y,z,yaw]
    ...
    def transformation_to_pose(self, transformation_matrix: np.ndarray) -> np.ndarray:
        """Convert 4x4 transformation matrix to pose vector [x,y,z,yaw] - ignoring roll and pitch"""
        # Extract translation
        ...
        # Only extract yaw angle, ignoring roll and pitch
        # Use atan2 on the rotation matrix elements that give yaw
    ...
    def pose_to_transformation(self, pose: np.ndarray) -> np.ndarray:
        """Convert pose vector [x,y,z,yaw] to 4x4 transformation matrix - only Z-axis rotation"""
        x, y, z, yaw = pose
        
        # Create rotation matrix only around Z-axis (yaw)
        # Roll = 0, Pitch = 0
    ...
    def predict_with_gicp(self, source_cloud, target_cloud, apply_gicp_func, cumulative_transform=None):
        ...
        MODIFIED: Only extracts 4DOF pose [x,y,z,yaw]
        Args:
        ...
        try:
            ...
            # Extract only 4DOF pose (x,y,z,yaw)
            ...
            print(f"GICP Prediction pose (4DOF, cumulative): {self.prediction_pose}")
    ...
    def update_with_gicp(self, source_cloud, target_cloud, apply_gicp_func, cumulative_transform=None):
        ...
        try:
            ...
            print(f"GICP Observation pose (4DOF, cumulative): {observation_pose}")
            ...
            if self.kalman_state is None:
                ...
                print("Initialized Kalman state with first observation (4DOF)")
            ...
            print(f"Kalman filtered pose (4DOF): {self.kalman_state.state}")
            return observation_pose, cumulative_transformation
    ...
    def get_current_pose(self) -> Optional[np.ndarray]:
        """Get current estimated pose (4DOF)"""
        if self.kalman_state is None:
            return None
...
def compare_to_ground_truth_positions(filtered_poses: List[np.ndarray], ground_truth_csv_path: str):
    ...
    MODIFIED: Updated for 4DOF poses [x,y,z,yaw]
    Args:
        filtered_poses: List of 4-DOF pose arrays [x,y,z,yaw]
    ...
    # Summary statistics
    print("\n=== Position Error vs Ground Truth (4DOF) ===")
    ...
    plt.ylabel("Error (m)")
    plt.title("Kalman Filter Position Error vs Ground Truth (4DOF - Z-axis rotation only)")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
...
def plot_dual_gicp_pose_evolution(prediction_poses, observation_poses, filtered_poses, step_size=1, step_info=""):
    ...
    Args:
        prediction_poses: List of GICP prediction poses (4DOF)
        observation_poses: List of CUMULATIVE GICP observation poses (4DOF, from reference)
        filtered_poses: List of Kalman filtered poses (4DOF)
    ...
    # Create figure with subplots (2x3 grid for 4 DOF + trajectory plots)
    fig = plt.figure(figsize=(18, 12))
    title = f'4DOF (Z-only rotation) CUMULATIVE Dual GICP Kalman Filter ({step_info}): Prediction vs Observation vs Filtered'
    fig.suptitle(title, fontsize=16)
    
    pose_labels = ['X (m)', 'Y (m)', 'Z (m)', 'Yaw (rad)']
    
    # Plot individual pose components (first 4 positions for 4 DOF)
    for i in range(4):
        ax = plt.subplot(3, 3, i+1)  # First 4 positions in 3x3 grid
        ...
        ax.set_title(f'{pose_labels[i]} Evolution (4DOF)')
        if i == 0:  # Only show legend on first plot to avoid clutter
            ax.legend()
        ax.grid(True, alpha=0.3)
    
    # Add X-Y trajectory plot (position 5 in 3x3 grid)
    ax_traj = plt.subplot(3, 3, 5)
    ...
    ax_traj.set_title('X-Y Trajectory (4DOF, Z-rotation only)')
    ax_traj.legend()
    ax_traj.grid(True, alpha=0.3)
    ax_traj.axis('equal')  # Equal aspect ratio for true trajectory shape
    
    # Add 3D trajectory plot (position 6 in 3x3 grid)
    ax_3d = plt.subplot(3, 3, 6, projection='3d')
    ...
    ax_3d.set_title('3D Trajectory (4DOF)')
    ax_3d.legend()
    
    # Add yaw angle visualization (position 7 in 3x3 grid)
    ax_yaw = plt.subplot(3, 3, 7)
    
    # Plot yaw angles over time
    obs_yaw = observation_array[:, 3]
    filt_yaw = filtered_array[:, 3]
    
    ax_yaw.plot(time_steps, np.degrees(obs_yaw), 'r-', label='GICP Observation', linewidth=2, marker='o', markersize=4)
    ax_yaw.plot(time_steps, np.degrees(filt_yaw), 'b-', label='Kalman Filtered', linewidth=3, marker='^', markersize=5)
    
    if len(prediction_array) > 0:
        pred_yaw = prediction_array[:, 3]
        pred_time_steps = [time_steps[j] for j in prediction_indices]
        ax_yaw.plot(pred_time_steps, np.degrees(pred_yaw), 'g--', label='GICP Prediction', linewidth=2, marker='s', markersize=4)
    
    ax_yaw.set_xlabel('File Index')
    ax_yaw.set_ylabel('Yaw Angle (degrees)')
    ax_yaw.set_title('Yaw Angle Evolution (Z-axis only)')
    ax_yaw.legend()
    ax_yaw.grid(True, alpha=0.3)
    
    # Add pose uncertainty visualization (position 8 in 3x3 grid)
    if len(valid_filtered) > 1:
        ax_uncert = plt.subplot(3, 3, 8)
...
def plot_trajectory_comparison(prediction_poses, observation_poses, filtered_poses, step_info=""):
    ...
    fig.suptitle(f'4DOF (Z-only rotation) CUMULATIVE Trajectory Analysis ({step_info})', fontsize=16)
    ...
    obs_yaw, filt_yaw = observation_array[:, 3], filtered_array[:, 3]
    
    # Plot 1: X-Y trajectory with orientation arrows
    ax1 = axes[0, 0]
    ...
    # Add orientation arrows for yaw angle (every few points)
    for i in range(0, len(obs_x), max(1, len(obs_x)//8)):
        arrow_length = 0.5  # Adjust based on your coordinate scale
        dx_obs = arrow_length * np.cos(obs_yaw[i])
        dy_obs = arrow_length * np.sin(obs_yaw[i])
        dx_filt = arrow_length * np.cos(filt_yaw[i])
        dy_filt = arrow_length * np.sin(filt_yaw[i])
        
        ax1.arrow(obs_x[i], obs_y[i], dx_obs, dy_obs, head_width=0.1, head_length=0.1, fc='red', ec='red', alpha=0.6)
        ax1.arrow(filt_x[i], filt_y[i], dx_filt, dy_filt, head_width=0.1, head_length=0.1, fc='blue', ec='blue', alpha=0.8)
    ...
    ax1.set_xlabel('X (m)')
    ax1.set_ylabel('Y (m)')
    ax1.set_title('X-Y Trajectory with Yaw Orientation (4DOF)')
    ax1.legend()
    ...
    ax2.set_ylabel('Z (m)')
    ax2.set_title('Altitude Profile (4DOF)')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    # Plot 3: Yaw angle over time
    ax3 = axes[1, 0]
    ax3.plot(time_steps, np.degrees(obs_yaw), 'r-', label='GICP Observation (Cumulative)', linewidth=2, marker='o', markersize=5)
    ax3.plot(time_steps, np.degrees(filt_yaw), 'b-', label='Kalman Filtered', linewidth=3, marker='^', markersize=6)
    ax3.set_xlabel('Time Step')
    ax3.set_ylabel('Yaw Angle (degrees)')
    ax3.set_title('Yaw Angle Evolution (Z-axis only)')
    ax3.legend()
    ...
    for i in range(len(valid_filtered)):
        ...
        pos_error = np.linalg.norm(filt_pos - obs_pos)
        position_errors.append(pos_error)
        
        # Angular error (handle wrap-around)
        yaw_diff = np.abs(observation_array[i, 3] - filtered_array[i, 3])
        yaw_diff = min(yaw_diff, 2*np.pi - yaw_diff)  # Handle wrap-around
        yaw_errors.append(np.degrees(yaw_diff))
    
    ax4_twin = ax4.twinx()
    line1 = ax4.plot(time_steps, position_errors, 'purple', linewidth=2, marker='d', markersize=5, label='Position Error')
    line2 = ax4_twin.plot(time_steps, yaw_errors, 'orange', linewidth=2, marker='s', markersize=5, label='Yaw Error')
    
    ax4.set_xlabel('Time Step')
    ax4.set_ylabel('Position Error (m)', color='purple')
    ax4_twin.set_ylabel('Yaw Error (degrees)', color='orange')
    ax4.set_title('Kalman Filter Errors (4DOF)')
    ...
    ax4.axhline(y=mean_pos_error, color='purple', linestyle='--', alpha=0.7)
    ax4_twin.axhline(y=mean_yaw_error, color='orange', linestyle='--', alpha=0.7)
    ...
    # Calculate total yaw rotation
    total_obs_yaw_change = np.sum([abs(obs_yaw[i] - obs_yaw[i-1]) for i in range(1, len(obs_yaw))])
    total_filt_yaw_change = np.sum([abs(filt_yaw[i] - filt_yaw[i-1]) for i in range(1, len(filt_yaw))])
    
    print(f"\n=== 4DOF (Z-only rotation) CUMULATIVE Trajectory Statistics ===")
    ...
    print(f"Total observed yaw change: {np.degrees(total_obs_yaw_change):.1f} degrees")
    print(f"Total filtered yaw change: {np.degrees(total_filt_yaw_change):.1f} degrees")
    print(f"Mean position error: {mean_pos_error:.3f} m")
    print(f"Mean yaw error: {mean_yaw_error:.1f} degrees")
    print(f"Max position error: {np.max(position_errors):.3f} m")
    print(f"Max yaw error: {np.max(yaw_errors):.1f} degrees")
    ...
    print(f"Total yaw rotation: {np.degrees(abs(obs_yaw[-1] - obs_yaw[0])):.1f} degrees")

# Rest of the functions remain the same, just need to update the process_point_clouds_with_kalman function
# to handle the new 4DOF poses properly
...
def process_point_clouds_with_kalman(observation_folder: str,
    ...
    MODIFIED: Updated for 4DOF poses [x,y,z,yaw] - Z-axis rotation only
    ...
    if apply_gicp_func is None:
        # You'll need to import the GICP functions from the original code
        from Kalman import apply_gicp_direct  # Replace with actual import
        apply_gicp_func = apply_gicp_direct
    
    # Load observation clouds with sampling and limit
    print("Loading observation point clouds...")
    # You'll need to import the load_pcd_files function from the original code
    from Kalman import load_pcd_files  # Replace with actual import
    ...
    print(f"\n=== 4DOF Processing Summary (Z-axis rotation only) ===")
    ...
    print(f"\nProcessing {len(observation_pcds)} observation clouds (every {observation_step_size}th) with 4DOF CUMULATIVE dual GICP Kalman filtering...")
    if use_separate_prediction_folder:
    ...
    for i, (obs_name, obs_cloud) in enumerate(observation_pcds):
        ...
        try:
            ...
            if use_separate_prediction_folder and prediction_pcds and i < len(prediction_pcds):
                ...
                from Kalman import generate_prediction_cloud  # Replace with actual import
            ...
            if prediction_cloud is not None:
                if use_separate_prediction_folder:
                    ...
                    print(f"Prediction GICP (cumulative, 4DOF): {obs_name} -> {pred_name}")
                else:
                    print(f"Prediction GICP (cumulative, 4DOF): {obs_name} -> generated prediction")
            ...
            if i < len(observation_pcds) - 1:  # Not the last observation
                ...
                print(f"Observation GICP (cumulative, 4DOF): {obs_name} -> {next_obs_name}")
                ...
                if len(observation_poses) > 0 and observation_poses[-1] is not None:
                    ...
                    observation_poses.append(np.array([0.0, 0.0, 0.0, 0.0]))  # Identity pose (4DOF)
            ...
            result = {
                ...
                'registration_type': 'cumulative_consecutive_4dof',  # Updated to reflect the change
                'dof': 4  # Indicate this is 4DOF
            }
            ...
            # Print summary
            if kalman_filter.prediction_pose is not None:
                print(f"Prediction pose (4DOF): {kalman_filter.prediction_pose}")
            if observation_pose is not None:
                print(f"Observation pose (4DOF, cumulative): {observation_pose}")
            if filtered_pose is not None:
                print(f"Filtered pose (4DOF): {filtered_pose}")
    ...
    if visualize and len(filtered_poses) > 0:
        ...
        from Kalman import visualize_final_registered_cloud  # Replace with actual import
        ...
        from Kalman import save_results  # Replace with actual import
...
# Example usage function for the modified 4DOF version
def main_4dof():
    """
    Example usage for 4DOF (Z-axis rotation only) - replace with your actual folder paths and GICP function
    """
    # Replace these paths with your actual folder paths
    observation_folder = "/home/robotics/testdata/charm"
    prediction_folder = "/home/robotics/testdata/charmpred"
    ...
    print("=== Scenario 1: 4DOF CUMULATIVE Limited clouds with separate prediction folder (10 obs, 15 pred) ===")
    results1, pred_transforms1, obs_transforms1, filtered_poses1, combined_cloud1 = process_point_clouds_with_kalman(
        observation_folder,
        prediction_folder,
        apply_gicp_direct,  # You'll need to import this from your original module
        visualize=True,
        ...
        max_observation_clouds=300,          # Limit to first 100 observation clouds
        max_prediction_clouds=300           # Limit to first 100 prediction clouds
    )

    compare_to_ground_truth_positions(filtered_poses1, "./output/eval/test1gt.csv")
    
    print(f"\n4DOF CUMULATIVE Processing complete!")
    ...
    print("Note: Only Z-axis rotation (yaw) is tracked, roll and pitch are ignored")
