...
def plot_non_repetitive_analysis(predicted_poses, observed_poses, final_poses, processor):
    ...
    title_suffix = " (Z=0 Mode)" if processor.force_z_zero else ""
    fig.suptitle(f'Non-Repetitive LiDAR Scan Analysis{title_suffix}', fontsize=16)
    ...
    for i in range(4):
        ...
        if row < 2 and col < 2:
            ...
            # Special handling for Z coordinate in Z=0 mode
    ...
    title_2d = '2D Trajectory (Z=0)' if processor.force_z_zero else '2D Trajectory'
    ax_traj.set_title(title_2d)
    ax_traj.legend()
    ...
    if motion_analysis and 'scan_count' in motion_analysis:
        ...
        if movements:
            ...
            movement_label = 'Movement (m) - XY only' if processor.force_z_zero else 'Movement (m)'
            ax_features.set_ylabel(movement_label)
...
def main_non_repetitive_lidar():
    ...
    # Configure paths
    observation_folder = "/home/robotics/testdata/BA80ac"  # Replace with your path
